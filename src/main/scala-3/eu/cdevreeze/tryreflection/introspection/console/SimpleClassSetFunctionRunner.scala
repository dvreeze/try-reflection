...
/**
 * ClassSetFunction runner. It takes 2 JSON config files (for runner and class set function), as well as a class path file.
 *
 * It forks program InternalSimpleClassSetFunctionRunner in a different OS process, enhancing its classpath.
 *
 ...
 *   Chris de Vreeze
 */
object SimpleClassSetFunctionRunner extends ManagedLogger:
  ...
  def main(args: Array[String]): Unit =
    require(
      args.sizeIs == 3,
      s"Usage: SimpleClassSetFunctionRunner <JSON config resource path> <ClassSetFunction JSON config resource path> <classpath file> "
    )
    ...
    val classSetFunctionConfigOption: Option[ClassSetFunctionFactories.Config] =
      parseJson(classSetFunctionConfigFile)
        .flatMap(_.as[ClassSetFunctionFactories.Config].toOption)
    require(classSetFunctionConfigOption.nonEmpty, "Could not interpret the ClassSetFunction JSON program parameter as Config")
    ...
    val mainClassName: String =
      "eu.cdevreeze.tryreflection.introspection.console.internal.InternalSimpleClassSetFunctionRunner"
    ...
    val javaCommand: Seq[String] =
      Seq(
        ...
        classSetFunctionConfigFile.toAbsolutePath.toString
      )
...
end SimpleClassSetFunctionRunner
