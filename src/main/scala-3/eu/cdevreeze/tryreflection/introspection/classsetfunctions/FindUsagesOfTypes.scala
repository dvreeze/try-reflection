...
/**
 * Class Set function searching for usage of the given types, in class/interface names, names of superclass or implemented interfaces,
 * method signatures, constructor signatures, field types, etc.
 *
 * @author
 *   Chris de Vreeze
 */
final class FindUsagesOfTypes(val classesToFind: Seq[Class[?]]) extends ClassSetFunctionReturningJson:

  private given constructorEncoder[A]: Encoder[Constructor[A]] = Encoder.encodeString.contramap(_.toGenericString)

  private given Encoder[Field] = Encoder.encodeString.contramap(_.toGenericString)

  private given Encoder[Method] = Encoder.encodeString.contramap(_.toGenericString)

  private given Encoder[Annotation] = Encoder.encodeString.contramap(_.toString)
...
object FindUsagesOfTypes extends ClassSetFunctionFactory[Json, FindUsagesOfTypes]:
