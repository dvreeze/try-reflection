...
object ClassSetFunctionFactories {
  ...
  final case class ConfigResolver(config: Config) {
    ...
    def resolveClassSetFunctionFactoryClass(
    ...
    ): Class[ClassSetFunctionFactory[Json, _ <: ClassSetFunctionReturningJson]] = {
      ...
      Using.resource(classHunter.findBy(searchConfig)) { searchResult =>
        searchResult.getClasses
          ...
          .get()
          .asInstanceOf[Class[ClassSetFunctionFactory[Json, _ <: ClassSetFunctionReturningJson]]]
      }
    }

    def resolveClassSetFunctionFactory(classHunter: ClassHunter): ClassSetFunctionFactory[Json, _ <: ClassSetFunctionReturningJson] = {
